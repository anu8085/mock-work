...
> ⚠️ **Mock dry-run repo.** This repository is hackathon *practice*. The official
> submission will be recreated in a brand-new public repository during the official
> Project Period. This repo is not the official entry.
...
- **Synthetic / open reference data only** — no real personal data.
- Never commit `.env`, `.venv`, `.local_state/`, SQLite DBs, keys, PATs, or passwords.
- Secrets are read from the environment / Databricks secrets and are never logged.

## Disclaimer

This tool provides **general information only** and is not legal, financial, or
benefits advice. Eligibility is determined by the relevant agency — always verify
directly with program administrators.

## License

Open-source license to be added in the official submission repository (see hackathon
